...
static json_t *create_fetch_response(HttpResponse *response, HttpHeaders *headers_data, long response_code, void *arena, arena_alloc_func alloc_func);
...
static struct curl_slist *configure_http_headers(CURL *curl, json_t *input, void *arena, arena_alloc_func alloc_func);
static char *build_url_with_query(const char *base_url, json_t *query_params, void *arena, arena_alloc_func alloc_func);

// Middleware interface functions (required to be non-static for dynamic loading)
int middleware_init(json_t *config);
json_t *middleware_execute(json_t *input, void *arena, arena_alloc_func alloc_func, arena_free_func free_func, const char *url_template, json_t *middleware_config, char **contentType, json_t *variables);
void middleware_destructor(void);
...
static long get_fetch_long_config(json_t *config, const char *key, long default_value) {
    ...
    if (value && json_is_integer(value)) {
        return (long)json_integer_value(value);
    }
...
static CURL *create_configured_curl(json_t *config, void *arena, arena_alloc_func alloc_func) {
    (void)arena; // Not used in this function
    (void)alloc_func; // Not used in this function
...
static void configure_http_method(CURL *curl, json_t *input, void *arena, arena_alloc_func alloc_func) {
    ...
    if (body_json) {
        // Convert JSON body to string using jansson
        body_data = json_dumps(body_json, JSON_COMPACT);
        if (body_data) {
            // Copy to arena
            size_t body_len = strlen(body_data);
            char *arena_body = alloc_func(arena, body_len + 1);
            if (arena_body) {
                memcpy(arena_body, body_data, body_len);
                arena_body[body_len] = '\0';
                // free(body_data); // Free the jansson-allocated string
                body_data = arena_body;
            } else {
                // free(body_data);
                body_data = NULL;
            }
...
// HTTP Headers Configuration - returns headers list that must be freed by caller
static struct curl_slist *configure_http_headers(CURL *curl, json_t *input, void *arena, arena_alloc_func alloc_func) {
    (void)arena; // Not used yet
    (void)alloc_func; // Not used yet
    
    json_t *headers_json = json_object_get(input, "fetchHeaders");
    if (!headers_json || !json_is_object(headers_json)) {
        return NULL;
    }
    ...
    json_object_foreach(headers_json, key, value) {
        if (json_is_string(value)) {
            ...
            size_t header_len = strlen(key) + strlen(val_str) + 3; // ": " + null terminator
            char *header_str = malloc(header_len);
            if (header_str) {
                snprintf(header_str, header_len, "%s: %s", key, val_str);
...
static json_t *create_fetch_response(HttpResponse *response, HttpHeaders *headers_data, 
                                   long response_code, void *arena, arena_alloc_func alloc_func) {
    (void)arena; // Not used yet
    (void)alloc_func; // Not used yet
    ...
    if (parsed_body) {
        ...
        json_object_set_new(result, "response", json_string(response->memory ? response->memory : ""));
    }
...
static bool is_url_allowed(const char *url, json_t *config) {
    ...
    // For now, just return true - can implement domain filtering later
    (void)url; // Suppress unused parameter warning
    return true;
}

// URL and Query Parameter Building (simplified for now)
static char *build_url_with_query(const char *base_url, json_t *query_params, 
    ...
    if (!query_params || !json_is_object(query_params) || json_object_size(query_params) == 0) {
        ...
        if (result) {
            memcpy(result, base_url, url_len);
            result[url_len] = '\0';
        }
        return result;
    }
    
    // For now, just return the base URL - can implement query building later
    size_t url_len = strlen(base_url);
    char *result = alloc_func(arena, url_len + 1);
    if (result) {
        memcpy(result, base_url, url_len);
...
static json_t *execute_http_request(const char *url, json_t *request_params, 
    ...
    json_t *timeout_json = json_object_get(request_params, "fetchTimeout");
    if (timeout_json && json_is_integer(timeout_json)) {
        long timeout = (long)json_integer_value(timeout_json);
        curl_easy_setopt(curl, CURLOPT_TIMEOUT, timeout);
    }
    
    // Build final URL with query parameters
    json_t *query_params = json_object_get(request_params, "fetchQuery");
    char *final_url = build_url_with_query(url, query_params, arena, alloc_func);
    if (!final_url) {
        curl_easy_cleanup(curl);
        return create_fetch_error("networkError", "Failed to build URL", url, 0);
    }
    ...
    // Configure headers and capture the list for cleanup
    struct curl_slist *headers_list = configure_http_headers(curl, request_params, arena, alloc_func);
